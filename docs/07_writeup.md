...
3. **Single OOD corpus.** HH-RLHF red-team is one distribution shift. AdvBench-style adversarial prompts, model-generated text, or in-the-wild deployment data may show different patterns. See also #4.
4. **Small token budget, short sequences, narrow ID corpus.** The ID cache is 102.4k tokens at 256-token sequences from Pile-10k; the OOD cache is 25.6k tokens at the same sequence length. Token volume is enough for stable AUC-ROC across all features but leaves AUC-PR CIs wide for the rarest one (ethics has only 75 test positives). 256-token sequences are also short for any "lookahead" claim; a refusal precursor that needs multi-turn context to flag wouldn't fit in our window. Three natural expansions, each within the Colab T4 budget: 1k-4k-token sequences (cheap, just increase `SEQ_LEN`), 1M-token caches (~30 min instead of 5), and additional ID corpora beyond web text (code, math, multilingual, conversational).
5. **Sparse layer coverage; one model size.** We probed 4 of Gemma-2-2B's 26 layers ({5, 8, 12, 20}). Dense coverage (every layer 0-25) would resolve whether the AUC-by-depth curve has a sharp phase-transition or climbs smoothly. More important, we tested only one model size; whether the precursor pattern is *layer-relative* (same absolute layer index across model sizes) or *depth-relative* (same fraction of network depth, which the broader scaling literature, e.g. Templeton et al. 2024, suggests) was not tested. Gemma-2-9B (42 layers) and Gemma-2-27B (46 layers) would settle this but need more compute than a Colab T4 for activation extraction.
6. **Token-level positives only.** We predict "does this token fire?" rather than "will the next 50 tokens be a refusal?"; a more practically interesting "lookahead" framing would need a different label.
7. **Probe is L2-logistic.** Linear was the right call here for three reasons: (i) the SAE itself is a linear projection + JumpReLU, so same-layer probing recovers exactly that linear signal; (ii) with 2304-dimensional residuals and a few hundred positives per feature, MLPs have more parameters than data and overfit before finding genuinely-nonlinear signal; (iii) linear scales: one matrix-vector multiply per token at deploy time, ~30 sec to train on CPU, weights are inspectable. Two settings where richer probes might still pay off, both unexplored: (a) **cross-layer pooling** — a small MLP head on top of concatenated residuals from {L5, L8, L12, L20} could pick up information no single-layer linear probe sees, particularly relevant for predicting layer-20 firings from earlier-layer activations alone; (b) **lookahead probes** — predicting feature firings several tokens in the future from current-token residuals is plausibly nonlinear since it requires composition across positions, not just within the residual at one position.
8. **M_SAE / N_probe is a token-budget headline, not an equal-task efficiency claim.** Re-stated: the SAE solves an unsupervised dictionary-learning problem; the probe is supervised binary classification. The ratio answers "how cheap is the labeled-predictor add-on once you have the SAE," not "is the SAE wasteful."
...
Alain, G., & Bengio, Y. (2016). Understanding intermediate layers using linear classifier probes. arXiv (Cornell University). https://doi.org/10.48550/arxiv.1610.01644

Belinkov, Y. (2022). Probing classifiers: Promises, shortcomings, and advances. Computational Linguistics, 48(1), 207-219.

Bricken, T., Templeton, A., Batson, J., Chen, B., Jermyn, A., Conerly, T., ... & Olah, C. (2023). Towards Monosemanticity: Decomposing Language Models With Dictionary Learning. (n.d.). Transformer-Circuits.pub. https://transformer-circuits.pub/2023/monosemantic-features
...
Lieberum, T., Rajamanoharan, S., Conmy, A., Smith, L., Sonnerat, N., Varma, V., ... & Nanda, N. (2024, November). Gemma scope: Open sparse autoencoders everywhere all at once on gemma 2. In Proceedings of the 7th BlackboxNLP Workshop: Analyzing and Interpreting Neural Networks for NLP (pp. 278-300).

Pedregosa, F., Varoquaux, G., Gramfort, A., Michel, V., Thirion, B., Grisel, O., ... & Dubourg, V. (2011). Scikit-learn: Machine learning in Python: the Journal of machine Learning research, v. 12.
...
Templeton, A., Conerly, T., Marcus, J., Lindsey, J., Bricken, T., Chen, B., ... & Henighan, T. (2024). Scaling Monosemanticity: Extracting Interpretable Features from Claude 3 Sonnet. (2024). Transformer-Circuits.pub. https://transformer-circuits.pub/2024/scaling-monosemanticity
...
Nanda, N. (2025, September 11). pile-10k. Huggingface.co. https://huggingface.co/datasets/NeelNanda/pile-10k

### Software and tooling

Bloom, J., Tigges, C., Duong, A., & Chanin, D. (2024). SAELens (Version 6.44.0) [Computer software]. GitHub. https://github.com/decoderesearch/SAELens

Lin, J. (2023). Neuronpedia: Interactive Reference and Tooling for Analyzing Neural Networks [Computer software]. https://www.neuronpedia.org
